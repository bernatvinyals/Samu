<?php 
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		if (isset($_POST["username"]) && isset($_POST["password"]) && isset($_POST["email"])) {
			...
			$queryemail = test_input($_POST["password"]);
			...
			$userResult = mysqli_query($conn, 'INSERT INTO users (username, pass, email, confCode, verified, avatarID) VALUES ("'.$queryusername.'", "'.$querypassword.'", "'.$queryemail.'", '.rand(1000,9999).', 0, 1)');
			if (!$userResult) {
				echo "Error:".mysqli_error($conn);
				header("Location: register.php");
				die(); //error page here
...
<html>
	...
	<body>
		 <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
		 	<p>LOGO HERE</p>
		 	<label>Username(Cannot be renamed):</label>
